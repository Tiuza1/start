...
# Tempo de execução de cada comando no Arduino (baseado nos dados medidos no jogo)
# ATK:        F3(124ms hold) + release + click(14ms) ≈ 145ms
# FULL_COMBO: F1(28ms) + F2(28ms) + F3(124ms hold) + release + click(14ms) ≈ 200ms
# mouse_left -> f3: 99.7ms  |  f3 -> mouse_left: 124.4ms  (medido no jogo)
FULL_COMBO_EXEC  = 0.200   # segundos que o Arduino leva para executar FULL_COMBO
ATK_EXEC         = 0.145   # segundos que o Arduino leva para executar ATK

# Espera adicional após o comando (reação do jogo / ícone da aura atualizar)
# mouse_left->f3 = 99.7ms => POST_CMD_WAIT ~ 80ms para ciclo total ~225ms
POST_CMD_WAIT    = 0.080   # ajuste conforme necessário

POLL_INTERVAL_OFF = 0.05   # frequência do loop com CAPS desligado
STATUS_LOG_EVERY  = 10     # loga status a cada N iterações (evita spam de log)
# ─────────────────────────────────────────────────────────────────────────────
...
def iniciar_janela_logs(stop_event, area_holder):
    ...
    root = tk.Tk()
    root_ref = root
    root.title('Ninja Combo Manager v2')
    ...
    tk.Label(
        top,
        text='CAPS ON -> checa aura -> FULL_COMBO (ausente) ou ATK (presente) -> loop',
        bg='#111111', fg='#00ff88', font=('Consolas', 10, 'bold')
    ).pack(side='left')
...
def executar(placa, stop_event):
    ...
    log('Script v2 iniciado.')
    log(f'FULL_COMBO_EXEC={FULL_COMBO_EXEC}s  ATK_EXEC={ATK_EXEC}s  POST_CMD_WAIT={POST_CMD_WAIT}s')
    log(f'Ciclo FULL_COMBO~{(FULL_COMBO_EXEC+POST_CMD_WAIT)*1000:.0f}ms  ATK~{(ATK_EXEC+POST_CMD_WAIT)*1000:.0f}ms')
        ...
        with mss.mss() as sct:
            while not stop_event.is_set():
                ...
                if caps != last_caps:
                    last_caps = caps
                    if caps:
                        log('CAPS LIGADO -> combo loop ativo')
                        iter_count = 0
                    else:
                ...
                # ── executar combo adequado ──────────────────────────────────
                if tem_aura:
                    enviar_comando(placa, 'ATK')
                    wait = ATK_EXEC + POST_CMD_WAIT
                    if iter_count % STATUS_LOG_EVERY == 0:
                        log(f'[{iter_count}] Aura OK ({score_aura:.2f}) -> ATK | espera={wait*1000:.0f}ms')
                    time.sleep(wait)
                else:
                    enviar_comando(placa, 'FULL_COMBO')
                    wait = FULL_COMBO_EXEC + POST_CMD_WAIT
                    if iter_count % STATUS_LOG_EVERY == 0:
                        log(f'[{iter_count}] Aura AUSENTE ({score_aura:.2f}) -> FULL_COMBO | espera={wait*1000:.0f}ms')
                    time.sleep(wait)
