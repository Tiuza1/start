...
def executar(placa, stop_event):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if g_hp < 100: 
                print(">>> [MORTE DETECTADA!] Aguardando 2s para o jogo processar a morte...")
                time.sleep(2.0) # Espera a animação de morte/tela de ressurreição abrir

                print(">>> Enviando Alt+6 e pausando bot...")
                # Ação de resgate (Alt+6)
                placa.write(b"X\n")
                placa.flush()
                time.sleep(1.0) # Delay para garantir o envio e processamento do item

                print(">>> Aguardando reviver (HP verde) e Num Lock ativado para continuar...")
                ...
                while True:
                    ...
                    # Se a vida voltou e o Num Lock está LIGADO, retoma
                    if g_hp_novo >= 100 and macro_autorizado():
                        print(">>> [RETOMANDO] Personagem vivo e Num Lock ativado. Voltando ao trabalho...")
                        break
            ...
            if contador >= 15:
                ...
                r_hp, g_hp, b_hp = pyautogui.pixel(hp_x, hp_y)
                if g_hp < 100: 
                    print(">>> [MORTE DETECTADA na KAFRA!] Aguardando 2s para o jogo processar a morte...")
                    time.sleep(2.0) # Espera a animação
