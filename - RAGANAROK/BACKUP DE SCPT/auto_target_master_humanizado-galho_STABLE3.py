        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                for contour in contours:
                    ...
                    if area > 100 and area < 4000:
                        ...
                        if M["m00"] != 0:
                            ...
                            tentativas_recheck = 0  # mob atacado, reseta contador de re-scan
                ...
                if not arvore_encontrada:
                    ...
                    tentativas_falhas = 0
                    if alvos_recentes:
                        # Atirou recentemente mas scan voltou vazio (mobs no TTL de 1.5s)
                        # Aguarda monstro morrer e re-escaneia antes de decidir teleportar
                        time.sleep(random.uniform(0.75, 0.85))
                        alvos_recentes.clear()       # libera mobs para serem re-detectados
                        tentativas_recheck += 1
                        if tentativas_recheck <= 2:  # permite até 2 re-scans
                            continue                 # volta ao topo: re-escaneia sem teleportar
                    # Chegou aqui: área limpa (nenhum tiro recente)
                    # OU esgotou 2 re-scans sem achar mais nada → teleporta
                    tentativas_recheck = 0
                    placa.write(b"T\n")
                    placa.flush()
