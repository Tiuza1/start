...
# SEM CLICK — usuario clica manualmente, script so pressiona teclas
# ATK:        F3(50ms hold) + soltar ≈ 60ms
# FULL_COMBO: F1(28ms) + F2(28ms) + F3(50ms) ≈ 120ms
FULL_COMBO_EXEC  = 0.120   # segundos que o Arduino leva para executar FULL_COMBO
ATK_EXEC         = 0.060   # segundos que o Arduino leva para executar ATK

# Espera adicional minima — usuario controla o ritmo com o click manual
POST_CMD_WAIT    = 0.030
...
def iniciar_janela_logs(stop_event, area_holder):
    ...
    root = tk.Tk()
    root_ref = root
    root.title('Ninja Combo Manager v3 [sem click]')
    ...
    tk.Label(
        top,
        text='CAPS ON -> checa aura -> teclas (SEM CLICK) -> voce clica manualmente',
        bg='#111111', fg='#ffaa00', font=('Consolas', 10, 'bold')
    ).pack(side='left')
    ...
    txt = ScrolledText(root, bg='black', fg='#ffaa00',
...
def executar(placa, stop_event):
    ...
    log('Script v3 iniciado [SEM CLICK - usuario clica manualmente].')
        ...
        with mss.mss() as sct:
            while not stop_event.is_set():
                ...
                if caps != last_caps:
                    last_caps = caps
                    if caps:
                        log('CAPS LIGADO -> loop ativo | clique manualmente para atacar')
                        iter_count = 0
                    else:
