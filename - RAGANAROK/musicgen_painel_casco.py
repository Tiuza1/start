...
class App(ctk.CTk):
    def __init__(self):
        ...
        self._build_ui()
        self._log("Painel carregado. AudioCraft/MusicGen integrado (CPU).")
    ...
    def _on_gerar(self):
        ...
        nome_resolvido = self._resolver_modelo(params["model_size"], params["mode"])
        self._log(
            f"Iniciando — modelo={nome_resolvido}  "
    ...
    @staticmethod
    def _resolver_modelo(size: str, mode: str) -> str:
        """Mapeia (tamanho, modo) → nome do modelo AudioCraft."""
        if mode == "melody":
            # melody-large existe; para small/medium usa-se musicgen-melody (médio)
            return "facebook/musicgen-melody-large" if size == "large" else "facebook/musicgen-melody"
    ...
    def _gerar_worker(self, p: dict):
        try:
            ...
            # --- importações pesadas só aqui para não travar a UI na abertura ---
            try:
                from audiocraft.models import MusicGen
                import torchaudio
            except ImportError as ie:
                raise RuntimeError(
                    f"AudioCraft não encontrado: {ie}\n\n"
                    "Instale com:\n"
            ...
            if self._stop_flag:
                self.after(0, lambda: self._log("Cancelado antes de carregar o modelo."))
                return

            nome_modelo = self._resolver_modelo(p["model_size"], p["mode"])

            # Reusa modelo em cache se for o mesmo (evita recarregar ~60s na CPU)
            if self._cached_model_name == nome_modelo and self._cached_model is not None:
                self.after(0, lambda: self._log("  Modelo já em memória — reutilizando."))
                model = self._cached_model
            else:
                self.after(0, lambda: self._log(
                    f"  Carregando {nome_modelo}… (primeira vez faz download, pode demorar)"
                ))
                model = MusicGen.get_pretrained(nome_modelo)
                self._cached_model      = model
                self._cached_model_name = nome_modelo
                self.after(0, lambda: self._log("  Modelo carregado."))
            ...
            if self._stop_flag:
                self.after(0, lambda: self._log("Cancelado após carregar modelo."))
                return

            self.after(0, lambda: self._log(
                f"  Gerando {p['duration']}s de áudio… (CPU: pode levar vários minutos)"
            ))

            if p["mode"] == "melody" and p["ref_audio"]:
                melody_wav, sr = torchaudio.load(p["ref_audio"])
            ...
            if self._stop_flag:
                self.after(0, lambda: self._log("Cancelado (geração concluída, descartando)."))
                return

            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            nome_arquivo = f"musicgen_{p['model_size']}_{p['duration']}s_{ts}.wav"
            out_path = p["out_dir"] / nome_arquivo
            torchaudio.save(str(out_path), wav[0].cpu(), model.sample_rate)

            self.after(0, lambda: self._on_geracao_concluida(str(out_path)))

        except Exception as exc:
            self.after(0, lambda exc=exc: self._on_geracao_erro(str(exc)))
