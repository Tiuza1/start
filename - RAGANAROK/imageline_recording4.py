...
class MacroManager:
    def __init__(self, root):
        ...
        status_row = tk.Frame(root, bg="#121212")
        status_row.pack(fill="x", padx=15, pady=(0, 4))
        self.lbl_status = tk.Label(status_row, text=status_txt, bg="#121212", fg=status_color, font=("Consolas", 10))
        self.lbl_status.pack(side="left", expand=True)
        _btn_txt = "DESCONECTAR" if self.placa else "CONECTAR"
        _btn_bg  = "#1a3a1a"     if self.placa else "#3a1a00"
        _btn_fg  = "#00ff44"     if self.placa else "#ff8800"
        self.btn_conexao = tk.Button(status_row, text=_btn_txt, command=self._toggle_conexao,
                                     bg=_btn_bg, fg=_btn_fg, font=("Arial", 8, "bold"),
                                     relief="flat", padx=8)
        self.btn_conexao.pack(side="right", padx=(5, 0))
    ...
    def _toggle_conexao(self):
        if self.placa and self.placa.is_open:
            self._desconectar_placa()
        else:
            self._conectar_placa()

    def _desconectar_placa(self):
        # Para todos os scripts em execução antes de fechar a porta
        ...
        try:
            ...
            print(f"[CONEXÃO] Erro ao fechar porta: {e}")
        self.placa = None
        self.lbl_status.config(text="PLACA DESCONECTADA — porta COM liberada", fg="#ff8800")
        self.btn_conexao.config(text="CONECTAR", bg="#3a1a00", fg="#ff8800")
        print("[CONEXÃO] Placa desconectada. Porta COM liberada.")

    def _conectar_placa(self):
        porta = encontrar_placa()
        if not porta:
            self.lbl_status.config(text="PLACA NÃO ENCONTRADA", fg="#ff0000")
            print("[CONEXÃO] Nenhuma placa encontrada.")
            return
        try:
            self.placa = serial.Serial(porta, 9600, timeout=0)
            joy_status = 'OK' if pygame.joystick.get_count() > 0 else 'N/D'
            self.lbl_status.config(text=f"PLACA: {porta} | JOYSTICK: {joy_status}", fg="#00ff00")
            self.btn_conexao.config(text="DESCONECTAR", bg="#1a3a1a", fg="#00ff44")
            print(f"[CONEXÃO] Conectado em {porta}.")
        except Exception as e:
            self.placa = None
            self.lbl_status.config(text=f"ERRO AO CONECTAR: {e}", fg="#ff0000")
            print(f"[CONEXÃO] Falha ao conectar: {e}")
    ...
    def toggle_script(self, nome):
        ...
        if not script['status']:
            ...
            try:
                ...
                print(f"[ERRO toggle_script] {err}")
                messagebox.showerror("Erro ao ligar script", f"{nome}\n\n{err}")
        else:
            script['status'] = False
            script['btn_onoff'].config(text="OFF", bg="#555")
