...
# =================================================================
# 3. IMPORTADOR AUTOMÁTICO DE CSV (CRUZAMENTO COM CEP E JSON)
# =================================================================
with st.expander("📦 Importar Planilha de Entregas (CSV)", expanded=False):
    ...
    if arquivo_csv is not None:
        ...
        # Validação de segurança para garantir que as colunas existem
        if 'Local' not in df.columns or 'CEP' not in df.columns:
            ...
            for index, row in df.iterrows():
                ...
                # 1. Extrai o número da quadra do texto ("Q 235", "QUADRA 54", etc)
                match_numero = re.search(r'(?:QUADRA|QD|Q)\s*(\d+)', local_dump, re.IGNORECASE)
                
                # 2. Extrai e valida o formato do CEP
                match_cep = re.search(r'(\d{5})[-.\s]?(\d{3})', cep_dump)
                
                if match_numero and match_cep:
                    numero = str(int(match_numero.group(1))) # Transforma '09' em '9'
                    cep_prefixo = match_cep.group(1)         # Ex: '72853'
                    cep_sufixo = int(match_cep.group(2))     # Ex: 280
                    
                    padrao_bairro = ""
                    nome_bairro_amigavel = ""
                    
                    # 3. FILTRO RIGOROSO DE BAIRRO POR CEP
                    if cep_prefixo in ['72853', '72856']:
                        # Parque Estrela Dalva IX e X (Pix, P. X, px...)
                        padrao_bairro = r'p[\.\s]*i*x+'
                        nome_bairro_amigavel = "Parque 9 ou 10"
                        
                    elif cep_prefixo == '72859':
                        if cep_sufixo <= 500:
                            # Mansões Parque 8 (PVIII, P8...)
                            padrao_bairro = r'p[\.\s]*(v\s*i{3}|8)'
                            nome_bairro_amigavel = "Mansões Parque 8"
                        else:
                            # Mansões Parque 7 (PVII, P7...)
                            padrao_bairro = r'p[\.\s]*(v\s*i{2}|7)'
                            nome_bairro_amigavel = "Mansões Parque 7"
                            
                    else:
                        # Se não for 72853, 72856 nem 72859: CEP de outro bairro (ex: 72850)
                        alertas_cep_invalido.append(f"{local_dump} - CEP: {cep_dump}")
                        continue # Pula para o próximo pacote
                    
                    # 4. Busca no Banco de Coordenadas
                    # Regra: Q + Numero + Variação do Sufixo permitida para este CEP
                    ...
                    if encontrou_no_banco:
                        sucessos.append(chave_encontrada)
                    else:
            ...
            # --- EXIBIR RESULTADOS ---
            if sucessos:
                st.success(f"✅ {len(sucessos)} locais encontrados e validados por CEP!")
            ...
            if alertas_nao_encontrado or alertas_sem_numero or alertas_cep_invalido:
                total_alertas = len(alertas_nao_encontrado) + len(alertas_sem_numero) + len(alertas_cep_invalido)
                st.warning(f"⚠️ {total_alertas} endereços bloqueados na validação (Mapeamento Manual):")
                
                if alertas_nao_encontrado:
                    st.error("❌ Quadra não mapeada no JSON para o bairro deste CEP:")
                    for alerta in alertas_nao_encontrado:
                        st.write(f"- {alerta}")
                        
                if alertas_cep_invalido:
                    st.error("🛑 CEP fora da área (Diferente de 72853/56/59) ou Inválido:")
                    for alerta in alertas_cep_invalido:
                        st.write(f"- {alerta}")
                        
                if alertas_sem_numero:
                    st.info("⚠️ Texto fora do padrão (Sem número da quadra e sem CEP pra deduzir):")
                    for alerta in alertas_sem_numero:
                        st.write(f"- {alerta}")
