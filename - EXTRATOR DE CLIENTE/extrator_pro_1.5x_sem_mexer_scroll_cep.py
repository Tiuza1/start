...
def extrair_logradouro(texto):
    if not texto:
        return "", "", ""
    rua = ""
    m = re.search(r'\bRUA\s+([A-Z0-9 ]+?)(?:\s*,|\s+\d|\s+QD|\s+QR|\s+Q\b|$)', texto, re.IGNORECASE)
    if m:
        rua = m.group(1).strip()
    partes_lote = []
    m = re.search(r'\b(LOTE?|LT)\.?\s*(\d+)', texto, re.IGNORECASE)
    if m:
        partes_lote.append(f"LOTE {m.group(2)}")
    m = re.search(r'\b(CASA|CS)\.?\s*(\d+)', texto, re.IGNORECASE)
    if m:
        partes_lote.append(f"CASA {m.group(2)}")
    lote = " ".join(partes_lote)
    apto = ""
        ...
        try:
            ...
            for i in range(len(codigos_nodes)):
                ...
                if meu_nome is not None and meu_btn is not None:
                    ...
                    rua, lote, apto = extrair_logradouro(txt_end)
