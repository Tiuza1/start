...
def rodar_extrator_v3_2():
    ...
    print("="*40)
    print(f"   EXTRATOR J&T V3.2 - {data_formatada}")
    print("="*40)
    ...
    try:
        while True:
            ...
            try:
                ...
                # Procura por todos os cards na tela
                ...
                for card in cards:
                    ...
                    # Percorre os itens DENTRO deste card específico
                    for child in card.iter('node'):
                        rid = child.get('resource-id')
                        if rid == ID_NOME: 
                            nome = child.get('text')
                        elif rid == ID_PEDIDO: 
                            id_pacote = child.get('text')
                        elif rid == ID_BOTAO_TEL: 
                            tel_bounds = child.get('bounds')
                        elif rid == ID_ENDERECO: 
                            txt_endereco = child.get('text')

                    # Só processa se o card estiver completo e não tiver sido processado
